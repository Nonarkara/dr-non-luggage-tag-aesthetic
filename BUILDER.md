...
The tag is the finished form of §2. It exists because a real, specific human has
eleven seconds, poor light, a torn strip, and no time — and every rank on the
face is a decision about that person. The PO Completeness gate is that model made
checkable: `node tools/tag-audit.mjs . --strict`.

Three bindings:

- **Register before anything** — [`docs/registers.md`](docs/registers.md). The
  tag answers eleven seconds. It is the wrong answer to an afternoon. Choosing
  the register is §2, not §8.
- **The audit is the machine half of §4, not the whole of it.** It checks
  hierarchy, ink limits, contrast, gradient structure, and whether an ephemeral
  mark records an event that happened. It cannot tell you the surface is good.
- **The skin is not decoration.** A printed tag has no barcode, so colour, scale
  and composition carry the sorting job — see
  [`docs/ephemera.md`](docs/ephemera.md). Same rule as §8's accent: it is
  load-bearing or it is out.
